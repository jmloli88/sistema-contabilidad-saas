    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Resumen financiero mes a mes</p>
            </div>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="{{ route('balances.index') }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 transition-colors duration-200">Balances</a>
                            </div>
                        </li>
                        <li>
                            <div class="flex items-center">
                                <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                                <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Mensual</span>
                            </div>
                        </li>
                    </ol>
                </nav>

                @include('balances.partials.filtros', [
                    'route' => route('balances.mensual'),
                    'filtros' => $filtros,
                    'clinicas' => $clinicas,
                ])
                ...
                @else
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 mx-4 sm:mx-0">
                        ...
                        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6">
                            <p class="text-sm font-medium text-gray-500 mb-1">Total Neto</p>
                            <p class="text-2xl font-bold text-gray-900">R$ {{ number_format($resumen['total_neto'], 2) }}</p>
                        </div>
                        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-6">
                            <p class="text-sm font-medium text-gray-500 mb-1">Margen Promedio</p>
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                                <div class="flex items-center space-x-3">
                                    <div class="bg-gradient-to-br from-blue-500 to-cyan-600 rounded-xl p-2.5">
                                        <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5 3a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2V5a2 2 0 00-2-2H5zm9 4a1 1 0 10-2 0v6a1 1 0 102 0V7zm-3 2a1 1 0 10-2 0v4a1 1 0 102 0V9zm-3 3a1 1 0 10-2 0v1a1 1 0 102 0v-1z" clip-rule="evenodd"></path></svg>
                                    </div>
                                    <h3 class="text-xl font-bold text-gray-900">Balance Mensual</h3>
                                </div>
                                <span class="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{{ $balances->count() }} meses</span>
                            </div>
                                    ...
                                    <div class="overflow-hidden">
                                        <table class="min-w-full text-sm text-left text-gray-700">
                                            <thead class="text-xs text-gray-700 uppercase bg-gradient-to-r from-gray-100 to-gray-50">
                                                <tr>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold whitespace-nowrap">Mes</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-right whitespace-nowrap">Ingresos</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-right whitespace-nowrap">Gastos</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-right whitespace-nowrap">Neto</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-right whitespace-nowrap">Margen</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-right whitespace-nowrap">Repases</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-right whitespace-nowrap">Consultas</th>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold text-center whitespace-nowrap">Detalle</th>
                                                </tr>
                                            </thead>
                                            <tbody class="divide-y divide-gray-200">
                                                @php
                                                    $totalIngresos = 0; $totalGastos = 0; $totalNeto = 0;
                                                    $totalRepases = 0; $totalConsultas = 0;
                                                @endphp
                                                ...
                                                @php
                                                    ...
                                                    $totalConsultas += $balance->total_consultas;
                                                    $parts = explode('-', $balance->period);
                                                @endphp
                                                <tr class="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td class="px-4 sm:px-6 py-4 text-right whitespace-nowrap {{ $balance->margen_ganancia !== null && $balance->margen_ganancia >= 0 ? 'text-green-600' : 'text-red-600' }}">
                                                    ...
                                                    <td class="px-4 sm:px-6 py-4 text-right font-medium whitespace-nowrap">{{ $balance->total_consultas }}</td>
                                                    <td class="px-4 sm:px-6 py-4 text-center whitespace-nowrap">
                                                        <a href="{{ route('balances.detalle', ['periodo' => 'mensual', 'anio' => $parts[0] ?? date('Y'), 'periodo_index' => ltrim($parts[1] ?? '1', '0') ?: '1', 'clinica_id' => $filtros['clinica_id'], 'estado' => $filtros['estado']]) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 transition-colors text-xs font-medium">
                                                            Ver
                                                        </a>
                                                    </td>
                                                </tr>
                                            ...
                                            <tfoot class="bg-gray-50 font-semibold">
                                                <tr>
                                                    ...
                                                    <td class="px-4 sm:px-6 py-4 text-right">{{ $totalConsultas }}</td>
