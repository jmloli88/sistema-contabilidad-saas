<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="font-bold text-2xl text-gray-800 leading-tight">Balance Trimestral</h2>
                <p class="text-sm text-gray-600 mt-1">Resumen financiero por trimestre</p>
            </div>
                ...
                <nav class="flex mb-8 mx-4 sm:mx-0" aria-label="Breadcrumb">
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                <svg class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                                <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2">Trimestral</span>
                            </div>
                        </li>
                    </ol>
                </nav>

                @include('balances.partials.filtros', ['route' => route('balances.trimestral'), 'filtros' => $filtros, 'clinicas' => $clinicas])
                    ...
                    <div class="bg-yellow-50 border-2 border-yellow-200 rounded-2xl p-6 mx-4 sm:mx-0 mb-8">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-yellow-700">No hay datos disponibles para los filtros seleccionados.</p>
                            </div>
                        </div>
                    </div>
                @else
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
                                <div class="flex items-center space-x-3">
                                    <div class="bg-gradient-to-br from-green-500 to-emerald-600 rounded-xl p-2.5">
                                        <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M3 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd"></path></svg>
                                    </div>
                                    <h3 class="text-xl font-bold text-gray-900">Balance Trimestral</h3>
                                </div>
                                <span class="text-xs font-medium text-gray-500 bg-gray-100 px-3 py-1 rounded-full">{{ $balances->count() }} trimestres</span>
                            </div>
                                    ...
                                    <div class="overflow-hidden">
                                        <table class="min-w-full text-sm text-left text-gray-700">
                                            <thead class="text-xs text-gray-700 uppercase bg-gradient-to-r from-gray-100 to-gray-50">
                                                <tr>
                                                    <th scope="col" class="px-4 sm:px-6 py-3 sm:py-4 font-bold whitespace-nowrap">Trimestre</th>
                                            ...
                                            <tbody class="divide-y divide-gray-200">
                                                ...
                                                @php
                                                    ...
                                                    $parts = explode('-Q', $balance->period);
                                                @endphp
                                                <tr class="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td class="px-4 sm:px-6 py-4 text-center whitespace-nowrap">
                                                        <a href="{{ route('balances.detalle', ['periodo' => 'trimestral', 'anio' => $parts[0] ?? date('Y'), 'periodo_index' => $parts[1] ?? '1', 'clinica_id' => $filtros['clinica_id'], 'estado' => $filtros['estado']]) }}" class="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors text-xs font-medium">Ver</a>
                                                    </td>
                                                </tr>
                                                @endforeach
                                            </tbody>
