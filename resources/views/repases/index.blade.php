        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-cyan-100 flex items-center justify-center flex-shrink-0">
                    <span class="material-symbols-outlined text-cyan-600 text-xl fill">payments</span>
                </div>
                <div>
                    <h2 class="font-bold text-xl text-gray-800 leading-tight">
                        Repases Médicos
                    </h2>
                    <p class="text-sm text-gray-500 mt-0.5">Gestiona los repases y pagos médicos</p>
                </div>
            </div>
            @if(Auth::user()->isAdmin())
                <a href="{{ route('repases.create') }}" class="flex-1 sm:flex-none bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded-xl text-sm inline-flex items-center justify-center gap-1.5">
